...
async function handleSubmit(): Promise<void> {
    ...
    try{
        ...
        errorMessage.value = error instanceof Error ? error.message : 'une erreur inconnue est survenue'
    } finally {
  ...
  <main class="flex min-h-screen items-center justify-center px-4 py-12">
    <section class="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl shadow-slate-200/70">
      <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold tracking-tight text-slate-900">Bienvenue</h1>
        <p class="mt-2 text-sm text-slate-600">Crée un compte pour retrouver tes habitudes.</p>
      </div>
      ...
      <form class="space-y-5" @submit.prevent="handleSubmit">
        ...
        >
          {{ isSubmitting ? 'En cours de creation...' : 'Créer un compte' }}
        </button>
      </form>
